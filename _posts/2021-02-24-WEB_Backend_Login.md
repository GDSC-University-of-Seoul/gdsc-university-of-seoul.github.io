...
부족한 부분도 많고 틀린 정보도 있을 수 있어서 틀린 부분 알려주시면 정말 감사하겠습니다.🥳

---

로그인 구현을 위해서 먼저 회원가입 구현을 했습니다.

1. 회원가입 구현

   처음 코드를 작성할 때는 어떻게 해야할지 너무 막막하여
   형진님이 올려주신 샘플 코드를 참고하며 `router` 파일 먼저 작성했습니다.

   클라이언트가 서버로 정보를 보내는 것이므로 `post` 메소드를 사용하고
...
4. 토큰 부여
   ...
   그림판으로 쓴 것 같지만...웹툰 작가의 꿈을 갖구 산 태블릿으로 열심히 적었습니다^^!
   악필은 노력해도 악필일 뿐...

   JWT는 크게 3가지 구조로 구분 됩니다.
